...
export default async function ListingDetail({ params }: { params: { slug: string } }) {
  ...
  const similar = (await getListings())
    .filter(l => l.id !== listing!.id && (l.propertyType === listing!.propertyType || l.address.city === listing!.address.city))
    .slice(0, 3);
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 max-w-6xl">
          <nav className="flex mb-6 text-sm" aria-label="Breadcrumb">
            <ol className="flex items-center space-x-2">
              ...
              <li>
                <Link href="/" className="text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400">Properties</Link>
              </li>
              <li className="text-gray-500">/</li>
          ...
          <Link href="/" className="inline-flex items-center text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            {[
              ...
              <div key={i} className="flex flex-col items-center justify-center p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-sm">
                ...
                <span className="text-2xl font-bold">{typeof value === 'number' ? value : value}</span>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6 mb-6">
                <div className="flex items-center mb-4">
                  <Image src={listing!.agent.photo} alt={listing!.agent.name} width={64} height={64} className="rounded-full object-cover mr-4" />
                  <div>
          ...
          {similar.length > 0 && (
            <div className="mt-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {similar.map((l)=>(
                  <Link key={l.id} href={`/listings/${l.slug}`} className="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
