...
export default function HomePage() {
  ...
  const onSearch = (f: SearchFilters) => {
    ...
    setTimeout(() => {
      ...
      if (f.priceMin) {
        const v = parseInt(f.priceMin);
        if (!isNaN(v)) res = res.filter((l) => l.price >= v);
      }
      if (f.priceMax) {
        const v = parseInt(f.priceMax);
        if (!isNaN(v)) res = res.filter((l) => l.price <= v);
      }

      if (f.beds !== 'Any') {
        const v = parseInt(f.beds);
        if (!isNaN(v)) res = res.filter((l) => l.bedrooms >= v);
      }
      if (f.baths !== 'Any') {
        const v = parseInt(f.baths);
        if (!isNaN(v)) res = res.filter((l) => l.bathrooms >= v);
      }

      if (f.propertyType !== 'Any') {
        res = res.filter((l) => l.propertyType === f.propertyType);
      }
  ...
  const onSort = (opt: string) => {
    setSort(opt);
    const s = [...filtered]; // prefer-const fixed
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
              ...
              <div className="flex items-center">
                <label htmlFor="sort" className="mr-2 text-gray-700 dark:text-gray-300 text-sm">
                  Sort by:
                </label>
                <select
                  id="sort"
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {Array.from({ length: 8 }).map((_, i) => (
                  <div
                    key={i}
                    className="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-sm h-full animate-pulse"
                  >
