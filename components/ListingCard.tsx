...
export default function ListingCard({ listing, index }: { listing: Listing; index: number; }) {
  ...
  const { slug, title, price, bedrooms, bathrooms, squareFeet, address, images, propertyType, listedDate } = listing;

  return (
    <motion.div className="h-full" variants={cardVariants} initial="hidden" animate="visible" whileHover="hover"
      transition={{ delay: (index ?? 0) * 0.1 }} layout>
      <div className="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-sm h-full flex flex-col transition-shadow hover:shadow-md">
        <div className="relative">
          <Link href={`/listings/${slug}`} className="block aspect-[4/3] overflow-hidden">
            <Image src={images[0].url} alt={images[0].alt} fill className="object-cover" sizes="(min-width: 1280px) 25vw, (min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw" />
          </Link>
          <div className="absolute top-4 left-4">
            <span className={`text-xs font-medium px-2 py-1 rounded-full ${getPropertyTypeColor(propertyType)}`}>{propertyType}</span>
          </div>
          <div className="absolute top-4 right-4">
            <motion.button whileTap={{ scale: 0.95 }} aria-label="Save to favorites"
          ...
          <div className="mt-auto">
            <div className="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-700">
              <div className="flex items-center space-x-4 text-sm text-gray-700 dark:text-gray-300">
                <span className="flex items-center"><Bed className="w-4 h-4 mr-1" />{bedrooms}</span>
                <span className="flex items-center"><Bath className="w-4 h-4 mr-1" />{bathrooms}</span>
                <span className="flex items-center"><Square className="w-4 h-4 mr-1" />{squareFeet}</span>
              </div>
              <div className="text-xs text-gray-500 dark:text-gray-400">{getTimeSinceListing(listedDate)}</div>
            </div>
          </div>
        </div>
      </div>
