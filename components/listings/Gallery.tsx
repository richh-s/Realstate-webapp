    ...
    <>
      <div className={`relative ${full ? 'fixed inset-0 z-50 bg-black' : 'rounded-2xl overflow-hidden'}`}>
        <div className={`relative w-full ${full ? 'h-screen' : 'aspect-[16/9]'}`}>
          <AnimatePresence initial={false} custom={dir}>
            ...
            >
              <Image
                ...
                fill
                className={full ? 'object-contain' : 'object-cover'}
                sizes="100vw"
                priority
              />
            </motion.div>
          ...
          <button
            onClick={prev}
            className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white rounded-full p-2 focus:outline-none"
          ...
          <button
            onClick={next}
            className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white rounded-full p-2 focus:outline-none"
          ...
          <button
            onClick={() => setFull((v) => !v)}
            className="absolute top-4 right-4 bg-black/30 hover:bg-black/50 text-white rounded-full p-2 focus:outline-none"
            ...
            {images.map((_, i) => (
              <motion.button
                ...
                className={`w-2 h-2 rounded-full focus:outline-none ${
                  current === i ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-700'
                }`}
